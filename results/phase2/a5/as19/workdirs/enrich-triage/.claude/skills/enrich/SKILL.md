...
## S1c — Contamination triage (work-list mode)

The third way in, added by D-119.2b. The batch comes from the KB's own
state: every doc sync marked `status: contaminated`. **S2–S5 run as
below** — same evidence discipline, same self-check, same PR. What
changes is the work list, the classification you owe per doc, and one
hard line about what a repair may touch.

A contaminated doc is not a broken doc. It is a doc whose ground moved
and which nobody has re-read since. Most of them turn out to be right;
some of them turn out to be wrong; the point of triage is that **you find
out which, one doc at a time, with the facts in front of you** — and that
the KB stops refusing to build on knowledge that was fine all along.

### The work list

```bash
python3 .claude/skills/enrich/worklist.py --kb ~/cl-steward/kb --batches
```

Deterministic, stdlib-only, no model calls. Per doc it gives you the
marker sync wrote (`contamination: {object, change, detail, path}` — and
`path` is the lineage route when the contamination was *inherited* rather
than direct), the contaminating object's facts **as the snapshot has them
now** including its `CHECK` constraints, whether the doc's `depends_on`
still resolves, whether it was ever `verified`, whether a golden expects
it, and which of the changed columns the doc actually talks about.

It sorts by severity — unresolved dependencies, then previously-certified
docs, then report-path docs, then the big shared causes — and `--batches`
groups them into batches of ten that **keep one cause together**. Take one
batch. A pull request is something a person reads end to end, so it should
tell one story: these docs, this change, this repair.

**The tool assembles; it does not classify.** Deciding whether a new
constraint confirms a paragraph or contradicts it is reading, and a regex
must not do it for you.

### Classify each doc — exactly one of three

**1. `confirms-prose`.** The change is something the doc already says
correctly, or says nothing about. Example: a `status` column gains
`CHECK (status IN ('processing','completed','failed'))` and the doc's enum
decoding already lists exactly those three, or the doc never mentions
`status` at all. The repair is the marker and the stamp:

```yaml
status: draft            # was: contaminated
contamination: null      # cleared — you re-read it and it holds
written_against_schema_hash: "sha256:<the current one>"
```

**The body does not change. Not one word.** If you found yourself
improving a sentence, this doc was not in this class — either it needed
re-grounding, or you are polishing under cover of a repair, and a
reviewer skimming twenty "no-change" repairs will not catch it. The
front-matter-only diff is what makes this batch reviewable at all.

**2. `needs-re-grounding`.** The change contradicts, narrows, or extends
what the doc says. Rewrite the affected claims from the current snapshot
and cite the new evidence at the tier it deserves — **a DB `CHECK` is
ground truth** (`app DDL` grade) and outranks a customer-stated value set
it contradicts. Say so where they disagree: "the doc listed `pending`;
the constraint added 2026-08-04 admits only `processing`, `completed`,
`failed`" is worth a sentence in Warnings, because the next reader will
otherwise wonder which of you was wrong.

Everything else about drafting is unchanged: CP-E4 (one-liners in
front-matter), the JSON rule, `depends_on` covering what you rely on.

**3. `depends-on-missing-object`.** A `depends_on` FQN no longer resolves
against the snapshot. **Do not repair this doc.** Leave it
`contaminated`, name it in the PR body with the decision it needs — drop
the dependency, or wait for the object to come back — and where that
decision belongs to somebody else, `flag_gap` it. Deleting a dependency
line to make a red doc green is not a repair; it is removing the tripwire
and calling the room safe.

### You re-ground. The steward certifies.

**Never write `status: verified`, never write `last_verified`** — CP-E3,
unchanged, and it is KB §5's rule too: `contaminated → verified` is a
human transition. Your repaired docs land as `draft`.

The certification is the steward's own act on the branch, under their own
name, before they merge (solo-operator mode, D-116.3 — that merge *is*
the act KB-7 always meant). So the PR body tells them exactly what they
are being asked to put their name on, per doc:
...
Commit that under your own identity. Docs you are not ready to certify:
leave them `draft` and say why in the merge comment — a draft repair is
still better than a contaminated one, and it does not pretend.
```

Thirty docs is the same act as one, thirty times. If that is too many to
read honestly, take a smaller batch — the count is yours to choose and
the reading is not skippable.
...
- `<issue-id>` "the churn number" — no object named and no metric doc
  ...
  **Returned** with that note (`return_request`), so it is `approved`
  again and waiting for the requester. *(Without the tool in your
  profile: say **handed back**, and name the issue id and note for the
  steward to move — match the claim to the act.)*

CL-Resolves: <issue-id-of-the-first>
CL-Resolves: <issue-id-of-the-second>
```

**For a triage batch (S1c), the body carries the classification table
instead**, one row per doc, plus the certification block above:
...
|---|---|---|---|
| `systems/…/exports.md` | confirms-prose | `CHECK (status IN …)` — the doc's enum already lists exactly these | front-matter only: marker cleared, hash refreshed |
| `systems/…/imports.md` | needs-re-grounding | the constraint admits six values; the doc listed four | Warnings + `column_purposes.status` re-grounded, DDL cited |
| `entities/user.md` | depends-on-missing-object | `depends_on` names `supabase.public.legacy_users`, absent from the snapshot | **nothing — left contaminated**, decision needed (drop it, or restore the object) |
```
