...
**The manifest is the only pin; the carry never writes a workflow file.** (Amendment, ruling D-96 task 2 — R-6 option (b), from security review #2 F7.) `VENDOR-MANIFEST.yaml` carries `wheel:` and `runtime_deps:`; the bootstrapped `kb-ci.yml` reads both at job time and installs what it finds there. It therefore contains no version and needs no edit when a wheel is carried. The reason is scope, not tidiness: while the carry repointed the workflow's install line, the sync identity needed **`workflow` write** on the customer's repository — a token that can rewrite CI is a token that can rewrite the thing that checks its own work, which is not what §9's "contents + pull-request write and nothing else" permits. With this amendment the wheel commit stages `.github/vendor/**` only (asserted by SO-10), and `workflow` scope comes off the sync PAT.

Two consequences worth stating rather than discovering: the carry must **preserve** `runtime_deps` verbatim (dropping it would leave CI installing no pinned dependencies at all), and a KB whose `kb-ci.yml` still hardcodes a wheel filename keeps working only until the next carry, at which point the install line names a file that is no longer there — **a loud CI failure, deliberately, not a silent validation against a stale wheel** (the D-46 failure mode). Migrating such a KB is a one-time PR against its workflow, made by a human, not by the sync identity.
...
| SO-F | Configured-but-disabled sync is silent in single-instance ops (motion filed under D-84.2: the pilot ran two days with `SYNC_ENABLED=0` after a compose env-precedence slip — drift accrued unpublished, nothing warned) | None — `/healthz` already reports `sync_enabled`; the gap is that nothing consumes it where there is no dashboard or alerting surface | A second silent-off incident, or the first customer deployment where the operator cannot eyeball the process env |
| SO-G | **Refresh cadence for GA4/GSC data delivered into an api-class publish target** (filed per D-94.1, authorized by the report-authoring spec's §12.5 register action; D-91.5 records the risk acceptance it comes from). Under the Power BI push-model leg the platform, not the source, owns the freshness of the data sitting in the semantic model: a delivered model is a point-in-time copy, and nothing re-delivers it. This is a sync-policy question — which systems get re-pulled, how often, and what re-delivery a stale model triggers — not a publisher question | None in v1: models are refreshed only by an explicit re-publish (a skill/agent-triggered revision, per RA-E). `scheduled_refresh: no` is declared on the Power BI connection's `publish.flags`, so the limitation is visible in the registration rather than implied | First standing report whose viewers depend on it being current — or the first customer question of the form "why is this report showing last week's numbers". RA-E (scheduled re-delivery) is the companion item on the authoring side; whichever fires first should carry both |
