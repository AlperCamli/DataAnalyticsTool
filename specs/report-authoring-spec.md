...
| RA-10 | **The session's Power BI access is scoped by the service principal, not by trust.** The SP is a member of the designated workspace(s) only; PBI-side RLS is not enforced under SP auth (Microsoft preview behavior) and is not relied upon — delivered data is already exec-role reporting-view aggregates. Recorded in the threat model. Since D-96 task 2 (CP-8 finding R-5) the preflight *asserts* the "only": the `sp-scope` check is green when the SP sees nothing beyond the configured workspace(s) and advisory-loud, naming the extras, when it does | Least privilege at the workspace boundary; no security property depends on a preview behavior — and membership-only is now measured, not promised |
...
| RA-F | Fabric/DirectLake escalation from the RA-6 push-model leg (the §12.5 register action, filed here as RA-6's row) | Push semantic models stay the v1 data plane; DirectLake/lakehouse is the additive escalation, not a v1 path | **Dated trigger (D-92.2, re-dated by D-96.3e):** Microsoft ends **new** push-API model creation on **2027-10-31**; existing models are unaffected. Decision due **2027-01-31**, or the first `push_limit_exceeded` capability failure at any estate (§5), or the second Power BI customer — whichever comes first. Tripwire (D-98 task 0): the publisher reports ≥80 %-of-limit proximity in its result detail and the core records it to health as `push_limit_proximity`, so approach is visible before the first refusal |
| RA-G | **Report lifecycle and teardown** (filed per D-94.1, the second of the two remaining §12.5 register actions). §5's publish path creates semantic models and reports and revises them in place, but nothing in v1 retires them: there is no rule for when a report is superseded, archived, or deleted from the workspace, no owner for orphaned models whose backing view was dropped, and no teardown of a demo/pilot workspace beyond deleting it by hand. Adjacent and unresolved with it: what a revision does to the previous definition hash's attestation row (kept, per L-5 immutability) versus what a *deletion* should write | None — reports and models persist until someone removes them in Power BI. `publish_report` has no `retire` mode; the attestation table is append-only and keeps every revision's row, so the audit story survives a hand-deletion even though the platform does not know it happened | The first estate where an unretired report outlives its backing view (a dropped reporting view leaves a model refreshing against nothing), or the first customer asking to delete a report through the platform rather than through Power BI. RA-D (naming/collisions) and SO-G (refresh cadence) are its neighbours — a lifecycle rule needs all three |
