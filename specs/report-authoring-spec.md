...
| RA-F | Fabric/DirectLake escalation from the RA-6 push-model leg (the §12.5 register action, filed here as RA-6's row) | Push semantic models stay the v1 data plane; DirectLake/lakehouse is the additive escalation, not a v1 path | **Dated trigger (D-92.2):** Microsoft ends **new** push-API model creation on **2027-10-31**; existing models are unaffected. The escalation therefore carries a deadline, not only a scale condition — decide no later than the first onboarding after mid-2027, or CP-8 of the next phase, whichever comes first. The scale condition stands alongside it: a `push_limit_exceeded` capability failure at any estate (§5) triggers the decision early |
