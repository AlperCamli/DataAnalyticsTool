...
| `table` | `checks` | array of strings | Engine-canonical CHECK-constraint definitions, verbatim (Postgres: `pg_get_constraintdef(oid, true)` over `pg_constraint` where `contype = 'c'`), lexicographically sorted; **included in hash**. SQL sources only — the API connectors have no analogue and must not emit the field |
...
**Registration record (SS-5 capture, ruling D-96.3d, applied 2026-07-31):** `checks` registered for `table` only, **hash-included**.

*Why hash-included, when `indexes` is not:* the S-2 test is "can this contradict a documented meaning?" An index cannot; a CHECK is exactly a documented meaning, expressed by the source. Dropping or widening `ai_runs_status_check` changes what the column can legally hold, so a doc saying "status is one of pending, completed, failed" becomes wrong — which is contamination, and must be able to reach the scan. Hash-included makes it breaking-diff-capable, which is the point of capturing it at all.

*Why verbatim strings and no parsing:* S-8 — snapshots carry facts, never synthesized prose. `pg_get_constraintdef` output is the source's own rendering of its own constraint. Parsing `status = ANY (ARRAY['pending', …])` into an enum set would be the connector inferring a vocabulary, which is the generator's and the human's job, not the boundary's. Consumers that want the vocabulary read the expression.

*Scope boundary, deliberately narrow:* `table` only — a view has no CHECKs of its own, and `materialized_view` is excluded for the same reason. `NOT NULL` stays out: it is already expressed as `columns[].nullable`, and Postgres exposes it as `contype = 'n'`, so filtering to `contype = 'c'` excludes it by construction rather than by convention. Domain (`contype` on the type) and exclusion (`contype = 'x'`) constraints stay out — SS-6/SS-7 territory, and **SS-6 is explicitly not pre-empted by this**: enum type labels remain Open and the two must not be bundled.

*Ordering:* lexicographic over the rendered strings, not catalog order, because `pg_constraint` order is not stable across dump/restore and S-3 requires the same source state to produce a byte-identical canonical body.

*The evidence that forced it (D-86.3b):* `deploy/reporting-views.sql` and D-81's rationale both called `ai_runs.status` "free text with no CHECK constraint" and "its vocabulary is ungrounded", when `ai_runs_status_check` enforces `pending | completed | failed`. A careful session read our boundary's silence as the source's permissiveness. That is the product's core failure mode — a grounded-sounding claim that is wrong because the grounding surface omitted the fact — and it happened in our own file, about our own estate.
...
| SS-5 | CHECK constraints in the snapshot | **CLOSED BY CAPTURE** (ruling D-96.3d, 2026-07-31). `stats.checks` registered on `table`, hash-included, verbatim `pg_get_constraintdef(oid, true)` over `contype = 'c'`, lexicographically sorted, Postgres only — see §4.5 and its registration record for the polarity argument, the scope boundary, and the evidence (D-86.3b: our boundary's silence was read as the source's permissiveness, in our own file, about our own estate). SS-6 is explicitly **not** pre-empted and stays Open | — |
