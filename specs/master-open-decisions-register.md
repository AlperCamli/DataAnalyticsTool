...
|---|---|---|---|---|
| E2 | Connections UI — the admin CLI has been its explicit stand-in since D-63.8 ("E2's Connections-UI stand-in"), running direct-DB | Open | Registration is a vendor CLI writing direct-DB; the CP-8 report graded playbook step 3 ASSISTED on exactly this, and the D-84 silent-failure pair (unregistered connection, unopened drift PR, both claimed done) is its cost | **B-2** (dashboard spec §9.4); the API it faces is A-3's |
| U-1 | Connections: list, register, configure, test, health | Open | Partial — jobs/health/snapshot endpoints exist; no connection CRUD endpoint | B-2 (API at A-3) |
| U-2 | Setup export / bundle delivery | Open | No served download (PA-1); `compileProfile` exists in-process only | B-3 (download served by A-2) |
| U-3 | Bundle staleness / compile-on-profile-change signal | Open | No compiled-at vs profile-changed-at record (PA-2) | B-3 (mechanism at A-2) |
...
| U-12 | Audit view + retention/export | Open | `audit_records` is written on every call; nothing serves it — `extract-audit.sh` is the workaround, and becomes a client of the B-0 endpoint | B-0 (API) / B-4 (view) |
