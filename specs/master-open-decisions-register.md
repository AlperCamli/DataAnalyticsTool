...
| U-12 | Audit view + retention/export | Open | `audit_records` is written on every call; nothing serves it — `extract-audit.sh` is the workaround, and becomes a client of the B-0 endpoint. **Scope gap filed 2026-08-06 (D-110.3a), home dashboard spec §5.1:** the table is one row per *MCP call*, so connection CRUD and every other governance write land in no audit row — the durable trace today is the job's `triggers` array, which exists only where a job exists. Widening the contract is a ruling, not a patch. **Scope gap CLOSED 2026-08-06 at B-1 (D-114.1)**, ahead of its trigger: the contract widened from "one row per MCP call" to "one row per governed act" with no schema change, and connection CRUD, verdicts, batches and returns now write rows — **denied included**. §5.1 no longer blocks B-4. D-107.3 (verdict history) and D-107.4 (jobs retention) are filed alongside it at §5.2 and **remain unfiled as register rows** | B-0 (API) / B-4 (view) |
