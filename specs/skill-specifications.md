...
**Not fixed, and named so (finding B1-F9):** the third per-item outcome — *return it to the queue* — is a governed **write**, and it has no session-reachable inlet (no MCP tool, no dashboard control). Until one exists, a request the skill cannot draft is named in the PR body's returned section **and handed back to the steward in words**, and CP-E5's ledger half (`batched → approved`, note recorded) is performed by whoever holds an API credential — the harness in AS-18, the operator otherwise. The skill must not pretend it returned something it could not.
